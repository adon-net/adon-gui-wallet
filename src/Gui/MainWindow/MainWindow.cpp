...
const char COMMUNITY_FORUM_URL[] = "https://t.me/adon_network";
const char REPORT_ISSUE_URL[] = "https://github.com/adon/adon-gui/issues";
...
MainWindow::MainWindow(ICryptoNoteAdapter* _cryptoNoteAdapter, IAddressBookManager* _addressBookManager,
  ...
  m_ui->setupUi(this);
  setWindowTitle(tr("AdonWallet %1").arg(Settings::instance().getVersion()));
  ...
  uiItems << m_ui->m_noWalletFrame << m_ui->m_overviewFrame << m_ui->m_sendFrame << m_ui->m_transactionsFrame 
          << m_ui->m_receiveFrame << m_ui->m_blockExplorerFrame << m_ui->m_addressBookFrame << m_ui->m_miningFrame << m_ui->statusBar;
...
void MainWindow::addRecipientTriggered() {
 RecepientPair recepient_pair = m_addRecipientAction->data().value<RecepientPair>();
  m_ui->m_sendFrame->addRecipient(recepient_pair);  m_ui->m_sendButton->click();
}
