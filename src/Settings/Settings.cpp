...
//
// Adon is free software: you can redistribute it and/or modify
// it under the terms of the GNU Lesser General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Adon is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU Lesser General Public License for more details.
//
// You should have received a copy of the GNU Lesser General Public License
// along with Adon.  If not, see <http://www.gnu.org/licenses/>.
...
const char DEFAULT_WALLET_FILE_NAME[] = "adon.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "pool.adon.network:3333";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("adon-wallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("adon.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("adon.desktop");
  ...
  res = autorunSettings.contains("adon") &&
    !QDir::fromNativeSeparators(autorunSettings.value("adon").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "network.adon.wallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=Adon Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("adon", appPath);
    } else {
      autorunSettings.remove("adon");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\adon", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:adon");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\adon\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\adon\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("adon-wallet.cfg"));
